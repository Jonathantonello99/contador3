  ...
  useEffect(()=>{
    ...
    const loop=(now)=>{
      ...
      try{
        ...
        const m=createMask(f);if(!m)return;const a=analyze(m),s=stabilize(a.total);setCount(s.value);setStabilizing(!s.stable);setAverageArea(Math.round(a.avgArea));setAverageDiameter(a.avgArea?2*Math.sqrt(a.avgArea/Math.PI):0);setDebugClusters(a.clusters.slice(0,12));render(f,m,a);m.delete();
